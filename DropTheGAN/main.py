...
def generation(input_path: str,
    ...
    input_image = utils_local.imread(input_path).to(device=device)
    ...
    utils_local.imwrite(output_path, output_image)
...
def editing(source_image_path: str,
    ...
    """
    device = _get_device(device)
    source_image = utils_local.imread(source_image_path).to(device=device)
    edited_image = utils_local.imread(edited_image_path).to(device=device)
...
def conditional_inpainting(masked_image_path: str,
    ...
    """
    device = _get_device(device)
    masked_image = utils_local.imread(masked_image_path).to(device=device)
    mask = utils_local.mask_read(mask_path).to(device=device)
...
def structural_analogy(source_image_path: str,
    ...
    """
    source_image = utils_local.imread(source_image_path).to(device=device)
    structure_image = utils_local.imread(structure_image_path).to(device=device)
